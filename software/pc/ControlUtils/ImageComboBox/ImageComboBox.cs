...
namespace ControlUtils.ImageComboBox
{
	[ToolboxItem(true)]
...
//	[Designer(typeof(System.Windows.Forms.Design.ControlDesigner))]
	...
	{
		#region Variables
		private DrawMode ComboBoxDrawMode = DrawMode.OwnerDrawVariable ; // set the default drawmode to owner drawselected.
		...
		private object[] DrawModes = new object[1];
		...
		public ImageComboBox()
		{
			...
			DrawModes[0] = DrawMode.OwnerDrawVariable ;
			base.ItemHeight = 15;
		...
		[DescriptionAttribute("Не используется. Для изменения режима рисования используйте свойство DrawModeFixed")] //щщщ
		[DesignerSerializationVisibility(DesignerSerializationVisibility.Visible)]
...
//		[EditorAttribute(typeof(CollectionEditor),typeof(UITypeEditor))]
		...
		private int editheight = 20;

		/// <summary>
		/// высота каждого элемента списка. Не используется при DrawModeFixed равном false
		/// ItemHeight теперь задает высоту объекта в нераскрытом состоянии
		/// </summary>
		[Browsable(true)]
		[Description("Высота элементов списка. ItemHeight задает высоту объекта в нераскрытом состоянии")]
		public int ItemsHeightReal
		{
			get
			{
				return editheight;
		...
		/// <summary>
		/// если true рисует все элементы с высотой заданной в свойстве ItemsHeightReal
		/// иначе высота каждого элемента задается отдельно свойством HeightOfItem или размером шрифта
		/// </summary>
		[Browsable(true)]
		[Description("Использовать ли режим фиксированного размера элементов списка")]
		public bool DrawModeFixed
		{
			get
			{
				return isfixed;
		...
		protected override void OnMeasureItem(MeasureItemEventArgs e)
		{
			...
			if (e.Index >= 0 && this.Items.Count > 0 && e.Index < this.Items.Count)
			{
				...
				if(!DrawModeFixed)
				{
					if (!this.Items[e.Index].NoAutoSizeToFont)
					{
						e.ItemHeight = (int) TextSize.Height;
					}
					else
					{
						e.ItemHeight = (int) Math.Max(TextSize.Height, this.Items[e.Index].HeightOfItem);
					}
				}
				else
				{
					e.ItemHeight = ItemsHeightReal;
				}
		...
		private void DrawItemHelperForItems(DrawItemEventArgs e)
		{
			...
			{	
				...
				int offset = 1;
				float HeightOfFont = e.Graphics.MeasureString(item.Text, item.Font).Height;
				int Y_offset = (int)(e.Bounds.Height - HeightOfFont) / 2;
				if(this.RightToLeft == RightToLeft.Yes)
				{
					ImageToDraw.RotateFlip(RotateFlipType.RotateNoneFlipX);
					format.Alignment = StringAlignment.Far;
				    RectangleF itemRect = new RectangleF((float)(e.Bounds.X - offset), (float)(e.Bounds.Y + Y_offset), (float)(e.Bounds.Width - IconWidth - indent - offset), (float)(HeightOfFont));//(e.Bounds.Height));
				...
				else
				{
					...
					RectangleF itemRect = new RectangleF((float)(e.Bounds.X + IconWidth + indent + offset), (float)(e.Bounds.Y + Y_offset), (float)(e.Bounds.Width - IconWidth - indent - offset), (float)(HeightOfFont));
					e.Graphics.DrawString (item.Text,item.Font,new SolidBrush (e.ForeColor),itemRect,format);
