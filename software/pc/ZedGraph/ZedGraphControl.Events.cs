...
namespace ZedGraph
{
	...
	#region Mouse Events
		...
		{
			if (_isAutoCursor && _masterPane != null )
			{
	...
	#region Mouse Wheel Zoom Events
					...
					bool isZoomOnCenter, bool isRefresh )
		{
			...
			if ( _isEnableHZoom )
			{
				if (zoomFraction < 1)
				{
					ZoomScaleKeepPointer(pane.XAxis, zoomFraction, x, isZoomOnCenter);
					ZoomScaleKeepPointer(pane.X2Axis, zoomFraction, x2, isZoomOnCenter);
				}
				else
				{
			...
			if ( _isEnableVZoom )
			{
				if (zoomFraction < 1)
				{
					for (int i = 0; i < pane.YAxisList.Count; i++)
						ZoomScaleKeepPointer(pane.YAxisList[i], zoomFraction, y[i], isZoomOnCenter);
					for (int i = 0; i < pane.Y2AxisList.Count; i++)
						ZoomScaleKeepPointer(pane.Y2AxisList[i], zoomFraction, y2[i], isZoomOnCenter);
				}
				else
				{
					for (int i = 0; i < pane.YAxisList.Count; i++)
		...
		protected void ZoomScale( Axis axis, double zoomFraction, double centerVal, bool isZoomOnCenter )
		{
			if ( axis != null && zoomFraction > 0.0001 && zoomFraction < 1000.0 )
			{
				...
				*/
				...
				if (range < scale._minRange)
					range = scale._minRange;

				if ( !isZoomOnCenter )
					centerVal = ( maxLin + minLin ) / 2.0;

				double _tmpMin = centerVal - range;
				double _tmpMax = centerVal + range;

				if (axis._scale.IsLimitMinScrollRange)
				{
					if (_tmpMin < axis._scale._minScrollRange)
					{
						_tmpMin = axis._scale._minScrollRange;
						//_tmpMax = axis._scale._maxScrollRange;
					}
				}

				if (axis._scale.IsLimitMaxScrollRange)
				{
					if (_tmpMax > axis._scale._maxScrollRange)
					{
						_tmpMax = axis._scale._maxScrollRange;
						//_tmpMin = axis._scale._minScrollRange;
					}
				...
				axis._scale._minLinearized = _tmpMin;
				axis._scale._maxLinearized = _tmpMax;
				//				}
		...
		protected void ZoomScaleKeepPointer(Axis axis, double zoomFraction, double centerVal, bool isZoomOnCenter)
		{
			if (axis != null && zoomFraction > 0.0001 && zoomFraction < 1000.0)
			{
				Scale scale = axis._scale;
...
//					centerVal = (maxLin + minLin) / 2.0;
				...
				double _tmpMin = centerVal - (centerVal - minLin) * zoomFraction;
				double _tmpMax = centerVal + (maxLin - centerVal) * zoomFraction;

				double range = (_tmpMax - _tmpMin);

				if (range < scale._minRange)
					_tmpMax = _tmpMin + scale._minRange;
	...
	#region Pan Events
		...
		protected void PanScale( Axis axis, double startVal, double endVal )
		{
			if ( axis != null )
			{
				...
				double _tmpMin, _tmpMax;
				double _tmpDelta;

				_tmpMin = scale._minLinearized + delta;
				_tmpMax = scale._maxLinearized + delta;

				if (scale._isLimitMaxScrollRange)
				{
					if (_tmpMax > scale._maxScrollRange)
					{
						_tmpMax = scale._maxScrollRange;
						_tmpMin = scale._minLinearized;
					}
				}

				if (scale._isLimitMinScrollRange)
				{
					if (_tmpMin < scale._minScrollRange)
					{
						_tmpMin = scale._minScrollRange;
						_tmpMax = scale._maxLinearized;
					}
				}

				scale._minLinearized = _tmpMin;
				scale._maxLinearized = _tmpMax;
		...
		private void HandleSelectionFinish( object sender, MouseEventArgs e )
		{
			...
			else   // It's a single-select
			{
				...
				using ( Graphics g = this.CreateGraphics() )
				{
					...
					{
						_item = nearestObj as CurveItem;

						if ( _item != null && _item._isSelectable && iPt >= 0 )
						{
							if ( Control.ModifierKeys == _selectAppendModifierKeys )
								_selection.AddToSelection( _masterPane, _item );
							else
								_selection.Select( _masterPane, _item );
						}
						else
