...
namespace ControlUtils.ImageComboBox
{
	...
	[Serializable()]
	[TypeConverterAttribute(typeof(ImageComboItemConverter))]
	...
	public sealed class ImageComboBoxItem : ISerializable
	{
		...
		[EditorAttribute(typeof(DropDownImages),typeof(System.Drawing.Design.UITypeEditor))]
		public string Image
		{
			get
			{
		...
		private bool Auto_size_to_font = true;

		/// <summary>
		/// Если true, то для задания высоты элемента используется значение HeightOfItem
		/// иначе размер задается исходя из размера шрифта
		/// </summary>
		[Browsable(true)]
		public bool NoAutoSizeToFont
		{
			get
			{
				return Auto_size_to_font;
			}
			set
			{
				Auto_size_to_font = value;
			}
		}

		private int Height_Of_Item;

		/// <summary>
		/// Высота элемента. Не используется при NoAutoSizeToFont равном false
		/// </summary>
		[Browsable(true)]
		[Description("Used only if \"NoAutoSizeToFont\" value is true")]
		public int HeightOfItem
		{
			get
			{
				return Height_Of_Item;
			}
			set
			{
				Height_Of_Item = value;
			}
		}
